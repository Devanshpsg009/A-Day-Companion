...
class LoginApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.geometry("900x600"); self.title("A Day Companion - Login"); self.resizable(False, False)
    ...
    def show_login_form(self):
        ...
        self.toggle_btn = ctk.CTkButton(pass_frame, text="SHOW", width=70, height=50, fg_color="#1e293b", hover_color="#334155", border_color="#334155", border_width=2, command=self.toggle_pwd)
        self.toggle_btn.pack(side="left")
        ctk.CTkButton(self.center_box, text="Forgot Password?", fg_color="transparent", text_color="#94a3b8", hover_color="#0f172a", command=self.show_forgot_form).pack(pady=(0, 10))
        ...
        footer = ctk.CTkFrame(self.center_box, fg_color="transparent"); footer.pack(pady=20)
    ...
    def show_forgot_form(self):
        ...
        self.reset_email_entry = ctk.CTkEntry(self.center_box, width=300, height=50, placeholder_text="Enter your email", fg_color="#1e293b", text_color="white")
        self.reset_email_entry.pack(pady=10)
        ctk.CTkButton(self.center_box, text="Verify Authenticator", width=300, height=50, fg_color="#3b82f6", command=self.action_verify_user).pack(pady=20)
    ...
    def action_update_password(self):
        ...
        if update_password(self.reset_email, new_pass): messagebox.showinfo("Success", "Password updated!"); self.show_login_form()
    ...
    def login_action(self):
        ...
        if authenticate_user(email, pwd):
            uid = get_user_id(email); self.destroy(); DashboardApp(uid).mainloop()
...
if __name__ == "__main__": LoginApp().mainloop()