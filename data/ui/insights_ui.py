...
class InsightsApp(ctk.CTkToplevel):
    ...
    def setup_tabs(self):
        self.tabs = ctk.CTkTabview(
            self, fg_color="#0f172a", segmented_button_selected_color="#3b82f6"
        )
    ...
    def setup_dev_tools(self):
        self.dev_frame = ctk.CTkFrame(
            self, height=50, corner_radius=0, fg_color="#020617"
        )
    ...
    def ensure_db_schema(self):
        with sqlite3.connect(self.todo_db) as conn:
            ...
            """)
            cols = [
                info[1] for info in conn.execute("PRAGMA table_info(todos)").fetchall()
            ]
            if "date" not in cols:
                try:
    ...
    def get_mood_stats(self, start_date, days_count):
        ...
        with sqlite3.connect(self.journal_db) as conn:
            while current <= end_date:
                ...
                """,
                    (self.user_id, current.isoformat()),
                ).fetchone()
    ...
    def split_frame(self, tab):
        ...
        ctk.CTkFrame(tab, width=2, fg_color="#334155").pack(
            side="left", fill="y", pady=20
        )
    ...
    def build_daily_tab(self):
        ...
        ctk.CTkLabel(
            right, text="Today's Mood", font=("Arial", 16, "bold"), text_color="#a855f7"
        ).pack(pady=10)
        with sqlite3.connect(self.journal_db) as conn:
            row = conn.execute(
                """
        ...
        canvas = ctk.CTkCanvas(
            right, width=200, height=200, bg="#0f172a", highlightthickness=0
        )
        canvas.pack(pady=30)
        circle_color = "#a855f7" if score_val > 0 else "#334155"
        canvas.create_oval(20, 20, 180, 180, outline=circle_color, width=8)
        canvas.create_text(
            100, 80, text=str(score_val), fill="white", font=("Arial", 60, "bold")
        )
        canvas.create_text(100, 130, text="/ 10", fill="gray", font=("Arial", 20))
        ctk.CTkLabel(
            right, text=mood_text, font=("Helvetica", 24, "bold"), text_color="white"
        ).pack(pady=10)
    ...
    def create_kpi(self, parent, title, value, color, side):
        ...
        ctk.CTkLabel(
            card, text=str(value), font=("Arial", 24, "bold"), text_color=color
        ).pack(pady=(0, 5))
    ...
    def embed_chart(self, parent, chart_type, data, labels, color):
        try:
            ...
            ctk.CTkLabel(
                parent, text="Chart unavailable - matplotlib issue", text_color="gray"
            ).pack(pady=20)
            return
