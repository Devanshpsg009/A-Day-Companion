...
class InsightsApp(ctk.CTkToplevel):
    def __init__(self, user_id):
        ...
        self.todo_db, self.journal_db = "todo.db", "journal.db"
        ...
        ctk.CTkLabel(self.header, text="Productivity & Wellness Insights", font=("Helvetica", 24, "bold"), text_color="white").pack(pady=15)
        ...
        self.tabs.add("Daily"); self.tabs.add("Weekly"); self.tabs.add("Monthly")
        ...
        self.dev_switch = ctk.CTkSwitch(self.dev_frame, text="Developer Mode", command=self.toggle_dev_tools, variable=self.dev_var, progress_color="#22c55e")
        self.dev_switch.pack(side="left", padx=20, pady=10)
        self.dev_tools = ctk.CTkFrame(self.dev_frame, fg_color="transparent")
        ctk.CTkButton(self.dev_tools, text="Inject Random Data", fg_color="#ef4444", hover_color="#b91c1c", width=150, command=self.inject_fake_data).pack(side="left", padx=10)

    def toggle_dev_tools(self):
        if self.dev_var.get(): self.dev_tools.pack(side="left", padx=10)
        else: self.dev_tools.pack_forget()

    def ensure_db_schema(self):
        with sqlite3.connect(self.todo_db) as conn:
            conn.execute("CREATE TABLE IF NOT EXISTS todos (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, task TEXT, time TEXT, status INTEGER DEFAULT 0, notified INTEGER DEFAULT 0, date TEXT)")
            cols = [info[1] for info in conn.execute("PRAGMA table_info(todos)").fetchall()]
            if 'date' not in cols:
                try:
                    conn.execute("ALTER TABLE todos ADD COLUMN date TEXT")
                    conn.execute("UPDATE todos SET date = ? WHERE date IS NULL", (datetime.date.today().isoformat(),))
                except: pass
        with sqlite3.connect(self.journal_db) as conn:
            conn.execute("CREATE TABLE IF NOT EXISTS journal (user_id INTEGER, date TEXT, content TEXT, tags TEXT, mood TEXT, score INTEGER, PRIMARY KEY (user_id, date))")

    def inject_fake_data(self):
        tasks, moods = ["Study AI", "Gym", "Code Python", "Read Book", "Meditation", "Project X"], ["Happy", "Sad", "Productive", "Tired", "Anxious", "Excited"]
        with sqlite3.connect(self.todo_db) as conn:
            for i in range(30):
                d_str = (datetime.date.today() - timedelta(days=i)).isoformat()
                for _ in range(random.randint(0, 5)):
                    conn.execute("INSERT INTO todos (user_id, task, date, status, notified) VALUES (?, ?, ?, ?, 1)", (self.user_id, random.choice(tasks), d_str, 1 if random.random() > 0.3 else 0))
        with sqlite3.connect(self.journal_db) as conn:
            for i in range(30):
                conn.execute("INSERT OR REPLACE INTO journal (user_id, date, content, mood, score) VALUES (?, ?, 'Fake Entry', ?, ?)", (self.user_id, (datetime.date.today() - timedelta(days=i)).isoformat(), random.choice(moods), random.randint(3, 9)))
        self.refresh_views()
    ...
    def get_task_stats(self, start_str, end_str):
        try:
            with sqlite3.connect(self.todo_db) as conn:
                total = conn.execute("SELECT count(*) FROM todos WHERE user_id=? AND date(date) BETWEEN date(?) AND date(?)", (self.user_id, start_str, end_str)).fetchone()[0]
                done = conn.execute("SELECT count(*) FROM todos WHERE user_id=? AND date(date) BETWEEN date(?) AND date(?) AND status=1", (self.user_id, start_str, end_str)).fetchone()[0]
            return total, done
        except: return 0, 0

    def get_mood_stats(self, start_date, days_count):
        data, labels, current, end_date = [], [], start_date, start_date + timedelta(days=days_count-1)
        with sqlite3.connect(self.journal_db) as conn:
            while current <= end_date:
                row = conn.execute("SELECT score FROM journal WHERE user_id=? AND date=?", (self.user_id, current.isoformat())).fetchone()
                data.append(row[0] if row else 0)
                labels.append(current.strftime("%a") if days_count == 7 else (str(current.day) if current.day % 3 == 0 else ""))
                current += timedelta(days=1)
        return data, labels

    def split_frame(self, tab):
        for w in tab.winfo_children(): w.destroy()
        left, right = ctk.CTkFrame(tab, fg_color="transparent"), ctk.CTkFrame(tab, fg_color="transparent")
    ...
    def build_daily_tab(self):
        ...
        with sqlite3.connect(self.journal_db) as conn:
            row = conn.execute("SELECT mood, score FROM journal WHERE user_id=? AND date=?", (self.user_id, today)).fetchone()
        mood_text, score_val = (row[0] if row and row[0] else "No Data"), (row[1] if row and row[1] else 0)
        
        canvas = ctk.CTkCanvas(right, width=200, height=200, bg="#0f172a", highlightthickness=0)
        canvas.pack(pady=30)
        canvas.create_oval(20, 20, 180, 180, outline="#a855f7" if score_val > 0 else "#334155", width=8)
    ...
    def build_weekly_tab(self):
        ...
        days, counts, current = [], [], start_date
        while current <= end_date:
            _, done = self.get_task_stats(current.isoformat(), current.isoformat())
            days.append(current.strftime("%a")); counts.append(done)
            current += timedelta(days=1)
    ...
    def build_monthly_tab(self):
        ...
        dates, counts, current = [], [], start_date
        while current <= end_date:
            _, done = self.get_task_stats(current.isoformat(), current.isoformat())
            dates.append(str(current.day) if current.day % 3 == 0 else ""); counts.append(done)
            current += timedelta(days=1)
    ...
    def embed_chart(self, parent, chart_type, data, labels, color):
        ...
        ax.tick_params(axis='x', colors='white', labelsize=8); ax.tick_params(axis='y', colors='white', labelsize=8)
        ax.spines['top'].set_visible(False); ax.spines['right'].set_visible(False)

        if chart_type == "pie":
            if sum(data) == 0: data, color = [0, 1], ["#22c55e", "#334155"]
            ...
            ax.set_xticks(x_indices); ax.set_xticklabels(labels)
            ax.grid(color='#334155', linestyle='--', linewidth=0.5)
            if color == "#a855f7": ax.set_ylim(0, 10)
