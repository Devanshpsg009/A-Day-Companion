...
from ui.digital_wellbeing import DigitalWellbeingApp, DigitalWellbeingDaemon
...
class DashboardApp(ctk.CTk):
    def __init__(self, user_id):
        ...
        try:
            self.tray_image = Image.open(os.path.join(ASSETS_DIR, "Ai.png"))
        except:
    ...
    def start_tray(self):
        if self.tray_running:
            return
        menu = pystray.Menu(
            pystray.MenuItem("Open", lambda *args: self.tray_queue.put("OPEN")),
            pystray.MenuItem("Quit", lambda *args: self.tray_queue.put("QUIT")),
        )
        self.tray_icon = pystray.Icon(
            "adaycompanion", self.tray_image, "A Day Companion", menu
        )
    ...
    def setup_main_dashboard(self, profile):
        ...
        self.time_label = ctk.CTkLabel(
            left, font=("Helvetica", 100, "bold"), text_color="#38bdf8"
        )
        self.time_label.pack(anchor="w")
        self.date_label = ctk.CTkLabel(
            left, font=("Helvetica", 24), text_color="#94a3b8"
        )
        self.date_label.pack(anchor="w", pady=(0, 30))
        self.streak_label = ctk.CTkLabel(
            left, text="", font=("Helvetica", 22, "bold"), anchor="w"
        )
        ...
        ctk.CTkLabel(
            ...
            wraplength=350,
            justify="left",
        ).pack(side="bottom", pady=20, anchor="w")
        ...
        self.load_assets()
        self.wellbeing_daemon = DigitalWellbeingDaemon(self.user_id)
        self.wellbeing_daemon.start()
        self.health_tracker = DigitalWellbeingApp(self.user_id)
        btns = [
    ...
    def update_streak_ui(self):
        ...
        if streak > 0:
            self.streak_label.configure(
                text=f"🔥 {streak } Day Streak", text_color="#f59e0b"
            )
        else:
            self.streak_label.configure(
                text="🔥 Start your streak!", text_color="#64748b"
            )
    ...
    def update_upcoming_ui(self):
        ...
        try:
            with sqlite3.connect("todo.db") as conn:
                tasks = conn.execute(
                    "SELECT task, time FROM todos WHERE user_id=? AND status=0 ORDER BY time ASC LIMIT 3",
                    (self.user_id,),
                ).fetchall()
            if not tasks:
                ctk.CTkLabel(
                    self.upcoming_frame, text="All caught up! 🎉", text_color="gray"
                ).pack(pady=15)
                return
            for task, time_str in tasks:
                ...
                ctk.CTkLabel(
                    r, text="•", text_color="#3b82f6", font=("Arial", 20)
                ).pack(side="left", padx=5)
                ctk.CTkLabel(r, text=task, text_color="white", anchor="w").pack(
                    side="left", fill="x", expand=True
                )
                if time_str:
                    ctk.CTkLabel(r, text=time_str, text_color="#94a3b8").pack(
                        side="right"
                    )
        except:
            pass
            ...
            with sqlite3.connect("todo.db") as conn:
                dates = [
                    ...
                    for r in conn.execute(
                        "SELECT DISTINCT date FROM todos WHERE user_id=? AND status=1 ORDER BY date DESC",
                        (self.user_id,),
                    ).fetchall()
                    if r[0]
                ]
    ...
    def load_assets(self):
        ...
        self.chess_img = load("chess.png")
        self.health_img = load("health.png")
    ...
    def show_profile_form(self):
        ...
        ctk.CTkLabel(
            card, text="Welcome to A Day Companion", font=("Helvetica", 24, "bold")
        ).pack(pady=(30, 10), padx=50)
        self.name_entry = ctk.CTkEntry(
            card, width=350, height=45, placeholder_text="Full Name (e.g.James)"
        )
        ...
        self.class_entry = ctk.CTkEntry(
            ...
            width=350,
            height=45,
            placeholder_text="Class/Grade (e.g. 10th/B.Tech 1st yr)",
        )
        self.class_entry.pack(pady=10)
        self.hobbies_entry = ctk.CTkEntry(
            card, width=350, height=45, placeholder_text="Hobbies"
        )
        self.hobbies_entry.pack(pady=10)
        self.goals_entry = ctk.CTkEntry(
            card, width=350, height=45, placeholder_text="Top Goals"
        )
    ...
    def save_profile_data(self):
        ...
        if name and cls:
            ...
            messagebox.showerror(
                "Required", "Please enter at least your Name and Class."
            )
