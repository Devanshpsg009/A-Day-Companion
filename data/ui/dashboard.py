...
class DashboardApp (ctk .CTk ):
    ...
    def start_tray (self ):
        """Start the system tray icon for the app."""
    ...
    def check_tray_queue (self ):
        """Check for messages from the tray icon."""
        try :
            while True :
    ...
    def open_ai_chat (self ):
        """Open the AI chat window if API key is available."""
        key =os .getenv ("OPENROUTER_API_KEY")
        AIChat (self .user_id ) if key else messagebox .showerror (
        "API Key Missing", 
        "AI features requires an API key."
        )

    def refresh_global (self ):
        """Refresh the global UI elements."""
        self .update_streak_ui ()
        self .update_upcoming_ui ()
    ...
    def update_upcoming_ui (self ):
        """Update the upcoming tasks display."""
    ...
    def show_profile_form (self ):
        ...
        ctk .CTkLabel (
        card ,
        text ="Welcome to A Day Companion",
        ...
        width =350 ,
        height =45 ,
        placeholder_text ="Full Name (e.g. James)"
        )
    ...
    def save_profile_data (self ):
        """Save the profile data and start the main dashboard."""
