...
class JournalApp(ctk.CTkToplevel):
    def __init__(self, user_id):
        ...
        self.current_font_family, self.current_font_size = "Georgia", 14
        self.init_db()
        ...
        for col in self.colors: ctk.CTkButton(toolbar, text="", width=25, height=25, fg_color=col, hover_color=col, corner_radius=5, command=lambda c=col: self.on_color(c)).pack(side="left", padx=3)
        ctk.CTkFrame(toolbar, width=2, height=30, fg_color="gray").pack(side="left", padx=10)
        ...
        self.text_editor = tk.Text(editor_frame, bg="#020617", fg="#e2e8f0", insertbackground="white", font=(self.current_font_family, self.current_font_size), selectbackground="#3b82f6", selectforeground="white", wrap="word", bd=0, highlightthickness=0)
    ...
    def setup_tags(self):
        ...
        for col in self.colors: self.text_editor.tag_configure(f"color_{col}", foreground=col)

    def run_analysis(self):
        content = self.text_editor.get("1.0", "end-1c")
        if len(content.split()) < 5: return messagebox.showwarning("Too Short", "Please write at least 5 words to analyze.")
        self.msg_label.configure(text="AI is thinking...", text_color="#eab308")
        self.update()
        result = analyze_sentiment(self.user_id, content)
        if result:
            mood, score, advice = result.get("mood", "Unknown"), result.get("score", 5), result.get("advice", "Keep writing!")
    ...
    def toggle_tag(self, tag_name):
        try:
            ...
            if tag_name in self.text_editor.tag_names("sel.first"): self.text_editor.tag_remove(tag_name, "sel.first", "sel.last")
            else: self.text_editor.tag_add(tag_name, "sel.first", "sel.last")
            self.save_entry()
        except: pass
    ...
    def on_color(self, color):
        try:
            if not self.text_editor.tag_ranges("sel"): return
            for c in self.colors: self.text_editor.tag_remove(f"color_{c}", "sel.first", "sel.last")
    ...
    def find_text(self):
        s = ctk.CTkInputDialog(text="Search for:", title="Find").get_input()
        if s:
    ...
    def on_key_release(self, event=None):
        content = self.text_editor.get("1.0", "end-1c")
        self.stats_label.configure(text=f"Words: {len(content.split()) if content.strip() else 0} | Chars: {len(content)}")
        self.save_entry()

    def save_entry(self, event=None, mood=None, score=None):
        content = self.text_editor.get("1.0", "end-1c")
        tags_data = [{"t": t, "s": str(r[i]), "e": str(r[i+1])} for t in ["bold", "italic"] + [f"color_{c}" for c in self.colors] for r in [self.text_editor.tag_ranges(t)] for i in range(0, len(r), 2)]
        with sqlite3.connect(self.db_path) as conn:
            if mood is None:
                row = conn.execute("SELECT mood, score FROM journal WHERE user_id=? AND date=?", (self.user_id, str(self.current_date))).fetchone()
                if row: mood, score = row
            conn.execute("INSERT INTO journal (user_id, date, content, tags, mood, score) VALUES (?, ?, ?, ?, ?, ?) ON CONFLICT(user_id, date) DO UPDATE SET content=excluded.content, tags=excluded.tags, mood=excluded.mood, score=excluded.score", (self.user_id, str(self.current_date), content, json.dumps(tags_data), mood, score))
        self.msg_label.configure(text="Saved", text_color="#22c55e")

    def load_entry(self):
        self.date_label.configure(text=self.current_date.strftime("%B %d, %Y"))
        self.text_editor.delete("1.0", "end"); self.mood_label.configure(text="")
        with sqlite3.connect(self.db_path) as conn:
            row = conn.execute("SELECT content, tags, mood, score FROM journal WHERE user_id=? AND date=?", (self.user_id, str(self.current_date))).fetchone()
        if row:
            self.text_editor.insert("1.0", row[0])
            if row[1]:
                try:
                    for item in json.loads(row[1]): self.text_editor.tag_add(item["t"], item["s"], item["e"])
                except: pass
            if row[2]: self.mood_label.configure(text=f"Mood: {row[2]} ({row[3]}/10)")
        self.on_key_release()
        self.msg_label.configure(text="Loaded", text_color="#38bdf8")

    def prev_day(self): self.current_date -= datetime.timedelta(days=1); self.load_entry()
    def next_day(self): self.current_date += datetime.timedelta(days=1); self.load_entry()