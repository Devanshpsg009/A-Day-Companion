...
class JournalApp(ctk.CTkToplevel):
    ...
    def setup_header(self):
        ...
        nav.pack(pady=10)
        ctk.CTkButton(
            nav, text="<", width=40, command=self.prev_day, fg_color="#334155"
        ...
        ctk.CTkButton(
            nav, text=">", width=40, command=self.next_day, fg_color="#334155"
        ).pack(side="left", padx=10)
    ...
    def setup_toolbar(self):
        ...
        ctk.CTkButton(
            ...
            fg_color="#334155",
            command=self.on_italic,
        ).pack(side="left", padx=2)
        ctk.CTkFrame(toolbar, width=2, height=30, fg_color="gray").pack(
            side="left", padx=10
        )
        ...
        ctk.CTkButton(
            ...
            width=30,
            fg_color="#334155",
            command=lambda: self.change_size(-2),
        ).pack(side="left", padx=2)
        ctk.CTkButton(
            toolbar,
            text="+",
        ...
        ctk.CTkButton(
            toolbar, text="Find", width=60, fg_color="#3b82f6", command=self.find_text
    ...
    def setup_status_bar(self):
        ...
        self.stats_label = ctk.CTkLabel(
            status, text="Words: 0 | Chars: 0", text_color="gray", font=("Arial", 12)
        )
        self.stats_label.pack(side="right")
        self.mood_label = ctk.CTkLabel(
            status, text="", text_color="#f472b6", font=("Arial", 12, "bold")
        )
        self.mood_label.pack(side="left", padx=10)
        self.msg_label = ctk.CTkLabel(
            status, text="Ready", text_color="#38bdf8", font=("Arial", 12)
        )
        self.msg_label.pack(side="left")

    def setup_tags(self):
        self.text_editor.tag_configure(
            "bold", font=(self.current_font_family, self.current_font_size, "bold")
        )
        self.text_editor.tag_configure(
            "italic", font=(self.current_font_family, self.current_font_size, "italic")
        )
        self.text_editor.tag_configure(
            "found", background="#f59e0b", foreground="black"
        )
        for col in self.colors:
    ...
    def run_analysis(self):
        ...
        if len(content.split()) < 5:
            return messagebox.showwarning(
                "Too Short", "Please write at least 5 words to analyze."
            )
        ...
        if result:
            ...
            messagebox.showinfo(
                f"AI Insight: {mood }", f"Score: {score }/10\n\n💡 Advice:\n{advice }"
            )
        else:
    ...
    def save_entry(self, event=None, mood=None, score=None):
        ...
        for tag_name in ["bold", "italic"] + [f"color_{c }" for c in self.colors]:
            ...
            for i in range(0, len(ranges), 2):
                tags_data.append(
                    {"t": tag_name, "s": str(ranges[i]), "e": str(ranges[i + 1])}
                )
        with sqlite3.connect(self.db_path) as conn:
            if mood is None:
                row = conn.execute(
