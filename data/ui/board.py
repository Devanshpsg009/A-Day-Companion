...
class ChessGame:
    def __init__(self, width, height, title, fps, engine_path, level="1500"):
        ...
        self.light_color = (220, 235, 255)
        self.dark_color = (100, 149, 237)
        ...
        try:
            self.capture_sound = pygame.mixer.Sound(
                os.path.join(ASSETS_DIR, "capture.wav")
            )
        except:
    ...
    def load_pieces(self):
        piece_paths = {
            "P_w": os.path.join(ASSETS_DIR, "Chess_plt60.png"),
            "P_b": os.path.join(ASSETS_DIR, "Chess_pdt60.png"),
            "B_w": os.path.join(ASSETS_DIR, "Chess_blt60.png"),
            "B_b": os.path.join(ASSETS_DIR, "Chess_bdt60.png"),
            "N_w": os.path.join(ASSETS_DIR, "Chess_nlt60.png"),
            "N_b": os.path.join(ASSETS_DIR, "Chess_ndt60.png"),
            "R_w": os.path.join(ASSETS_DIR, "Chess_rlt60.png"),
            "R_b": os.path.join(ASSETS_DIR, "Chess_rdt60.png"),
            "Q_w": os.path.join(ASSETS_DIR, "Chess_qlt60.png"),
            "Q_b": os.path.join(ASSETS_DIR, "Chess_qdt60.png"),
            "K_w": os.path.join(ASSETS_DIR, "Chess_klt60.png"),
            "K_b": os.path.join(ASSETS_DIR, "Chess_kdt60.png"),
        }
        loaded = {}
    ...
    def draw_highlights(self):
        if self.selected_square and not self.game_over:
            ...
            s.fill((0, 150, 0))
            self.screen.blit(
                s, (render_c * self.square_size, render_r * self.square_size)
            )
            ...
            for move in valid_moves:
                move_r, move_c = (
                    ...
                    else (7 - move[0], 7 - move[1])
                )
                pygame.draw.circle(
                    self.screen,
                    (0, 0, 255),
                    (move_c * self.square_size + 50, move_r * self.square_size + 50),
                    15,
                )
            ...
            for col in range(8):
                ...
                if piece != "--":
                    ...
                    if key in self.pieces:
                        r, c = (
                            (row, col) if self.choice == "white" else (7 - row, 7 - col)
                        )
                        self.screen.blit(
                            self.pieces[key],
                            (c * self.square_size + 5, r * self.square_size + 5),
                        )
    ...
    def animate_move(self, start, end):
        ...
        for frame in range(frame_count + 1):
            ...
            self.screen.blit(
                piece_image, (c * self.square_size + 5, r * self.square_size + 5)
            )
    ...
    def run(self):
        while True:
                    ...
                    if self.game_engine.turn == self.game_engine.player_color:
                        ...
                        if len(self.player_clicks) == 2:
                            ...
                            if end in self.game_engine.get_legal_moves(start):
                                ...
                                moving_piece = self.game_engine.board[start[0]][
                                    start[1]
                                ]
                                is_en_passant = (
                                    moving_piece[1] == "P"
                                ...
                                if (
                                    self.game_engine.checkmate()
                                    or self.game_engine.stalemate()
                                    or self.game_engine.threefold_repetition()
                                ):
                                    self.game_over = True
                            self.selected_square = ()
                            self.player_clicks = []
            if (
                not self.game_over
                and self.ai
                and self.game_engine.turn != self.game_engine.player_color
            ):
                self.draw_board()
                self.draw_pieces()
                pygame.display.update()
