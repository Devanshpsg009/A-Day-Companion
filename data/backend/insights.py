...
def has_started(user_id):
    ...
    with sqlite3.connect(DB_NAME) as conn:
        row = conn.execute(
            "SELECT started FROM streaks WHERE user_id=?", (user_id,)
        ).fetchone()
...
def start_insights(user_id):
    ...
    with sqlite3.connect(DB_NAME) as conn:
        ...
        conn.execute(
            "INSERT OR IGNORE INTO insights (user_id, date, opened) VALUES (?, ?, 1)",
            (user_id, today_str),
        )
...
def log_app_open(user_id):
    ...
    with sqlite3.connect(DB_NAME) as conn:
        row = conn.execute(
            "SELECT current_streak, last_opened FROM streaks WHERE user_id=?",
            (user_id,),
        ).fetchone()
        ...
        conn.execute(
            "UPDATE streaks SET current_streak=?, last_opened=? WHERE user_id=?",
...
def get_weekly_data(user_id):
    ...
    with sqlite3.connect(DB_NAME) as conn:
        rows = conn.execute(
            "SELECT date, opened FROM insights WHERE user_id=? AND date >= ? AND date <= ? ORDER BY date",
            (user_id, start.isoformat(), end.isoformat()),
        ).fetchall()
    data_map = {r[0]: r[1] for r in rows}
    days = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]
    return [
        (days[i], data_map.get((start + datetime.timedelta(days=i)).isoformat(), 0))
        for i in range(7)
    ]
...
def get_monthly_data(user_id):
    ...
    with sqlite3.connect(DB_NAME) as conn:
        rows = conn.execute(
            "SELECT date, opened FROM insights WHERE user_id=? AND date >= ? ORDER BY date",
            (user_id, start.isoformat()),
        ).fetchall()
    data_map = {r[0]: r[1] for r in rows}
    return [
        (
            str((start + datetime.timedelta(days=i)).day) if i % 2 == 0 else "",
            data_map.get((start + datetime.timedelta(days=i)).isoformat(), 0),
        )
        for i in range(30)
    ]